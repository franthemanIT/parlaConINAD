...
stampa("Sono pronto per inviare la lista dei codici fiscali a INAD.")
...
if os.path.exists(".\\token_INAD.py"):
    ...
    if int((adesso - allora).total_seconds()) < (durataToken-600):
        ...
        stampa("Il token a disposizione è ancora valido")
    else:
        stampa("Il token a disposizione è scaduto. Ne ottengo un altro.")
        ...
        if token_response.status_code == 200:
            ...
            File.write("\n")
            #a = input("Ho creato il token o voucher. Premi INVIO per inviare la lista di codici fiscali a INAD...")
            stampa("Ho creato il token (o voucher). Imvio la lista di codici fiscali a INAD...")
        else:
            stampa("Non sono riuscito a creare il token, guarda un po\' token_response cosa dice..")
else:
    stampa("Nessun token disponibile. Ne ottengo uno.")
    assertion = parlaConINAD.create_m2m_client_assertion(datiINAD.kid, datiINAD.alg, datiINAD.typ, datiINAD.iss, datiINAD.sub, datiINAD.aud, key, datiINAD.PurposeID)
    #with open("assertion.m2m", "w+") as file_ass:
        #file_ass.write(assertion)
